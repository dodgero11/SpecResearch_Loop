...
export class ClarifyService {
  ...
  async understand(projectId: string, idea: string, feedback?: string) {
    ...
    const keyIssues = Array.isArray(output.key_issues) ? output.key_issues.map(String) : [];
  ...
  async questions(projectId: string) {
    ...
    await this.prisma.confirmationQuestion.deleteMany({ where: { projectId, answeredAt: null } });
    const response = await this.ai.generateQuestions(clarification.clarifiedIdea);
    const rawQuestions = Array.isArray(response.output.questions) ? response.output.questions : [];
    const questions = [];
    for (const raw of rawQuestions) {
      ...
      questions.push({ id: created.id, title: created.title ?? created.question, example: created.example, options: created.options });
    }
    return { questions };
  }
  ...
  async answer(projectId: string, answers: ClarifyAnswerInput[]) {
    ...
    for (const answer of answers) {
      const question = await this.prisma.confirmationQuestion.findFirst({ where: { id: answer.questionId, projectId } });
      if (!question) {
        throw new NotFoundException(`Confirmation question ${answer.questionId} was not found`);
      }
      await this.prisma.confirmationQuestion.update({
        where: { id: answer.questionId },
        data: { selectedIndex: answer.selectedIndex, customAnswer: answer.customAnswer, answeredAt: new Date() },
      });
    }
    return { saved: true };
  }
