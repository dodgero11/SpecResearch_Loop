...
export class LocalAiGateway implements AiGateway {
  ...
  async decompose(context: Record<string, unknown>): Promise<AiGatewayResponse> {
    const types = ['PROBLEM', 'RESEARCH_QUESTION', 'GAP_CANDIDATE', 'CONTRIBUTION', 'CLAIM', 'EVIDENCE', 'CONSTRAINT', 'OPEN_QUESTION'];
    return {
      output: {
        cards: types.map((type) => ({ type, content: `Nội dung thẻ ${type}`, status: 'PROPOSED' })),
      },
  ...
  async relatedWorks(problem: string, researchQuestion: string, keywords?: string[]): Promise<AiGatewayResponse> {
    return {
      ...
      inputTokens: problem.length + researchQuestion.length + (keywords?.join(',').length ?? 0),
      outputTokens: 1,
    };
  }
  ...
  async gapAnalysis(gapCandidate: string, relatedWorks: unknown[]): Promise<AiGatewayResponse> {
    return {
      output: {
        what_was_done: 'Các nghiên cứu trước đã tiếp cận vấn đề theo hướng tổng quát.',
        limitation: 'Chưa tối ưu ở mức claim–evidence.',
        why_it_matters: 'Giảm hallucination khi trích xuất thông tin từ paper.',
        testable_with: 'So sánh tỉ lệ unsupported claims trên cùng tập dữ liệu.',
        directions: [
          { letter: 'A', label: 'Thuật toán tối ưu prompt', description: 'Tối ưu prompt theo vòng lặp.' },
          { letter: 'B', label: 'Claim–evidence verifier', description: 'Xác minh claim với evidence.' },
          { letter: 'C', label: 'Human-in-the-loop', description: 'Kết hợp xác nhận của người dùng.' },
          { letter: 'D', label: 'Kết hợp', description: 'Kết hợp các hướng trên.' },
        ],
      },
  ...
  async specExperiment(problem: string, gap: string, direction?: string): Promise<AiGatewayResponse> {
    return {
      output: {
        ...
        claims: [
          { claim: 'Claim 1', baseline: 'Baseline 1', metric: 'Metric 1', evidence: 'Evidence 1', rejection_condition: 'Điều kiện bác bỏ 1' },
          { claim: 'Claim 2', baseline: 'Baseline 2', metric: 'Metric 2', evidence: 'Evidence 2', rejection_condition: 'Điều kiện bác bỏ 2' },
        ],
        experiments: [
          { name: 'TN1', protocol: 'So sánh baseline', expected_outcome: 'Giảm unsupported claims' },
        ],
  ...
  async conflicts(claimEvidencePairs: unknown[], relatedWorks: unknown[]): Promise<AiGatewayResponse> {
    ...
    const conflicts = pairs.map((pair) => {
      ...
      return {
        ...
        linked_sources: Array.isArray(relatedWorks) && relatedWorks.length > 0 ? [relatedWorks[0]] : [],
        reason: 'Bằng chứng hiện tại chưa đủ để xác nhận claim, có nguy cơ mâu thuẫn với related work.',
      };
    });
    return { output: { conflicts }, inputTokens: JSON.stringify(claimEvidencePairs).length, outputTokens: conflicts.length };
  }
}