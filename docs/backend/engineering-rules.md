...
- Retry only timeouts, transient provider errors, and rate limits.
...
## Completion gates
Before a task is marked complete, run `npm test`, `npm run lint`, and `npm run build` from `backend/`. Acceptance criteria must map to executable tests.
